...
export default function MiDocumentacionPage() {
  ...
  const handleGuardar = async () => {
    ...
    if (!proveedor?.carpeta_drive_id) {
      setGlobalErr('Tu proveedor no tiene carpeta demo configurada. Contacta a administracion.')
      return
    }
    ...
    for (const key of keys) {
      ...
      try {
        ...
        newErrores[key] = err.message || 'Error demo.'
      }
    ...
    if (allOk) setGlobalOk('Todos los archivos se guardaron correctamente.')
    else if (someOk) setGlobalOk('Algunos archivos se guardaron. Revisa los errores.')
    else setGlobalErr('No se pudo guardar ningun archivo.')
  }
  ...
  return (
    <Box sx={{ maxWidth: 760, mx: 'auto' }}>
      ...
      <Typography sx={{ color: 'rgba(255,255,255,0.45)', fontSize: 13, mb: 3 }}>
        Carga tus documentos para enviarlos directamente a la carpeta demo asignada.
      </Typography>
      ...
      {proveedor && !proveedor.carpeta_drive_id && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Tu proveedor no tiene carpeta demo configurada. Contacta a administración.
        </Alert>
      )}
