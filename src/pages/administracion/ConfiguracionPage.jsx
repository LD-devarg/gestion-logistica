...
export default function ConfiguracionPage() {
  ...
  const [telegramStatus, setTelegramStatus] = useState({ configured: false, bot_username: 'LOGIDEMO_bot', api_ok: false, api_username: '', webhook_url: '', pending_update_count: 0 })
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const [u, p, t] = await Promise.all([
        ...
        client.get('/maestros/telegram/status/').catch(() => ({ data: { configured: false, bot_username: 'LOGIDEMO_bot', api_ok: false, api_username: '', webhook_url: '', pending_update_count: 0 } })),
      ])
      setUsuarios(u.data)
  ...
  const probarTelegram = async (p) => {
    ...
    try {
      await client.post(`/maestros/proveedores/${p.id}/telegram-test/`, {
        message: 'Mensaje de prueba de LOGIDEMO. Si lo recibiste, Telegram quedo configurado correctamente.',
      })
            ...
            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
              ...
              <Box>
                ...
                <Typography sx={{ color: 'rgba(255,255,255,0.45)', fontSize: 12, mt: 0.4 }}>
                  Canal de comunicacion con proveedores mediante @{telegramStatus.api_username || telegramStatus.bot_username || 'LOGIDEMO_bot'}.
                </Typography>
              </Box>
            </Box>
            <Chip
