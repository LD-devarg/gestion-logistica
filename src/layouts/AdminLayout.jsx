    ...
    return (
      <>
        <Tooltip title={collapsed ? item.label : ''} placement="right">
          ...
          >
            <ListItemIcon sx={{ color: '#fff', minWidth: collapsed ? 0 : 32 }}>
              {item.icon}
            </ListItemIcon>
            {!collapsed && (
              <>
    ...
    <NavLink to={item.to} style={{ textDecoration: 'none' }} onClick={onNavigate}>
      {({ isActive }) => (
        <Tooltip title={collapsed ? item.label : ''} placement="right">
          ...
          >
            <ListItemIcon sx={{ color: isActive ? '#60a5fa' : '#fff', minWidth: collapsed ? 0 : 32 }}>
              {item.icon}
            </ListItemIcon>
            {!collapsed && (
              <ListItemText
...
export default function AdminLayout() {
  ...
  const sidebarContent = (
    <>
      {/* Logo / Header */}
      <Box sx={{ display: 'flex', alignItems: 'center', px: isCollapsed ? 1 : 1.5, py: 1.5, gap: 1, justifyContent: isCollapsed ? 'center' : 'flex-start' }}>
        <Box
          sx={{
            width: 30,
            height: 30,
        ...
        >
          <LogisticaIcon sx={{ color: '#fff', fontSize: 18 }} />
        </Box>
        {!isCollapsed && (
      ...
      {/* User + Logout */}
      <Box sx={{ px: isCollapsed ? 0 : 1.5, py: 1.5, display: 'flex', alignItems: 'center', gap: 1, justifyContent: isCollapsed ? 'center' : 'flex-start' }}>
        <Tooltip title={isCollapsed ? (user?.username || 'Usuario') : ''} placement="right">
          <Avatar sx={{ width: 28, height: 28, bgcolor: '#3b82f6', fontSize: 12, flexShrink: 0, cursor: 'default' }}>
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#0f172a' }}>
      ...
      {/* â”€â”€ Desktop sidebar (permanent, hover to expand) â”€â”€ */}
      {!isMobile && (
        <Drawer
          variant="permanent"
      ...
      {/* â”€â”€ Mobile drawer (temporary) â”€â”€ */}
      {isMobile && (
        <Drawer
          variant="temporary"
          open={mobileOpen}
